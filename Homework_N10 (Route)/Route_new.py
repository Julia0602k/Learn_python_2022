...
# midle_speed = (int(v[0]) - int(v[1])) / (t - float(v[2]))
...
#         choise_m = input(f'''Ваши сохраненные маршруты: {route_is(0)[::3]}
# Напишите свой выбор или "New", чтобы добавить новый: ''')
#         print(f'''Чтобы проехать маршрут "{speed_middle(choise_m)[1]}" за {speed_middle(choise_m)[2]}ч
# вам нужно ехать по трассе со скростью {int(speed_middle(choise_m)[0])}-{int(speed_middle(choise_m)[0]) + 1}км/ч''')
    # elif == 3:
