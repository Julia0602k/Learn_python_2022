...
num1 = 0
v_korzine = dict.fromkeys(dict1.keys())
print(v_korzine)
sum_total = 0
while num1 != 'n':
        ...
        for k, v in dict1.items():
            if k == product:
                if num1 == '1':
                    ...
                    weight = input(f'Введите количество товара в граммах или для возврата в главное меню введите 0: ')
                    if weight.isdigit():
                        if weight == '0':
                            break
                        elif int(weight) > int(v[2]):
                            print(f'Ошибка! В наличии есть только {v[2]} грамм(-ов)')
                            continue
                        elif int(weight) <= int(v[2]):
                            sum_total += float(v[1]) * int(weight) / 100
                            print(f'\nВ корзине товар "{k}", масса: {weight} грамм(-ов), сумма за данное количество товара: {float(v[1]) * int(weight) / 100} рублей')
                            # if product not in v_korzine.keys():
                            #     v_korzine[key] = int(weight)
                            # else:
                            #     v_korzine[key] += int(weight)
                            # dict1[k] = int(v[2]) - int(weight)
